...
export class InstanceManager extends EventEmitter {
  ...
  public startInstance(
    id: string,
    operationToken?: string
  ): Promise<{ success: boolean; terminalSessionId?: string }> {
    return this.enqueueMutation(() => this.startInstanceInternal(id, operationToken))
  }
  ...
  public async restartInstance(
    ...
    try {
      ...
      const phase1 = await this.enqueueMutation(async () => {
        ...
        if (instance.status === 'running') {
          return { mode: 'stop', ...this.stopInitiateSerial(id, operationToken) } as const
        }
      ...
      return this.enqueueMutation(async () => {
        ...
        const result = await this.startInstanceInternal(id, operationToken)
   ...
   */
  private stopInitiateSerial(
    id: string,
    operationToken?: string,
    skipOperationLock = false
  ): { instance: Instance; terminalSessionId: string } {
    const instance = this.instances.get(id)
    if (!instance) {
      throw new Error('实例不存在')
    }
  ...
  private async stopInstanceInternal(id: string): Promise<InstanceTerminalLifecycleResult> {
    const captured = this.stopInitiateSerial(id, undefined, true)
    try {
      if (await this.waitForTerminalRelease(captured.instance, captured.terminalSessionId, 10_000)) {
  ...
  public async closeTerminal(
    ...
    const init = await this.enqueueMutation(async () => {
      const instance = this.instances.get(id)
      if (!instance) {
        throw new Error('实例不存在')
      }
      this.assertOperationLockOwner(id, operationToken)
      return this.closeInitiateSerial(id)
    })
    try {
  ...
  public async cleanup(): Promise<void> {
    ...
    await Promise.allSettled(runningInstances.map(instance => {
      ...
      return this.stopInstanceInternal(instance.id).catch(error => {
